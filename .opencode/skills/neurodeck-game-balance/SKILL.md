...
## CRUCIBLE ECONOMY: фляги только за осколки (10💰, макс 5); лавка: недостающие артефакты S=25/A=15/B=8💰. Фляги пьются и ВНЕ боя из рюкзака (кнопка 🧪, +50% maxHp, −1 фляга) — валюта ретрая после провала рана. Провал рана блокирует атаки (bossRunLocked) до фляги или понедельника. Perf-режимы: пять (auto/eco/performance/low/effects-off), eco≡low, performance≡effects-on. Фляги пьются и ВНЕ боя из рюкзака (кнопка 🧪, +50% maxHp, −1 фляга) — валюта ретрая после провала рана. Perf-режимы: пять (auto/eco/performance/low/effects-off), eco≡low, performance≡effects-on.

...
const SH = require('../../js/stronghold-data.js').STRONGHOLDS.map(function(d) { return { n: d.name, gar: d.gar, def: d.def, tot: d.total, tax: d.tax }; });
...
const TIERS = [
  ...
  { name: 'Хранитель Терна', cost: 525,  power: 450,  growth: 6 },   // [BALANCE] прирост 4→6; круг 3: без изменений (0.563); КРУГ 10: 800→525 — немонотонный стык закрыт, eff 0.857 = цель ×1.1 круга 3

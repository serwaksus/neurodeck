...
async function main() {
  ...
  try {
    ...
    l1('1f-призраки', 'ночь 2: призраки живут — списания НЕТ (код) vs «−1💰 за ночь» (UI/SPEC)', 0, ghost.night2,
      'списание только в ночь перехода active→ghost, далее призрак просто ждёт ghostDays (2/3/5 по tier)');

...
// ----------------------------------------------------------------
// Ф3: контракты интеграции + кэш v73
// ----------------------------------------------------------------
test('wave3 Ф3 контракты [css/style.css + index.html]: башня в renderStrongholds, экшн-кейс, CSS .tower-card, index.html ?v=73 ×8', () => {
    ...
    const html = htmlSource();
    assert.equal((html.match(/v=73/g) || []).length, 8, 'index.html: 8 вхождений ?v=73');
...
test('package.json: check:ui-скрипт верификации UI-ассетов подключён, JSON валиден', () => {
    ...
    assert.ok(htmlSource().includes('id="totemCard"') && (htmlSource().match(/v=73/g) || []).length === 8, 'index.html: PASS (totemCard + v73×8)');
